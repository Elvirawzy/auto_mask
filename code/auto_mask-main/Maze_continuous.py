...
class Gridworld(object):
    def __init__(self,
        ...
        self.action_space = gym.spaces.Discrete(2**n_actions)
        self.observation_space = gym.spaces.Box(low=0, high=1, shape=(8,), dtype=np.uint8)
        ...
        self.collision_reward = -0.1  # -0.05
    ...
    def render(self, path):
        ...
        if not self.disp_flag:
            ...
            self.currentAxis = plt.gca()                        #Turns off the the boundary padding
        ...
        if len(self.angles) > 0:
            r = np.array(self.curr_state[-self.n_lidar:])
        ...
        plt.savefig(path + '/'+ str(self.steps_taken)+ '.png')
        # plt.pause(1e-7)
    ...
    def save2gif(self, path, path2):
        dir_names = os.listdir(path)
        frames = []
        for dir_name in dir_names:
            frames.append(Image.open(path+'/'+dir_name))
        frames[0].save(path2+'.gif', save_all=True, append_images=frames,loop=100,duration=1)
        for dir_name in dir_names:
...
if __name__=="__main__":
    # Random Agent
    rewards_list = []
    env = Gridworld(debug=True, n_actions=4)
    for i in range(50):
        rewards = 0
        done = False
        # dir_name = str(Path(__file__).resolve().parents[0])+'/media/episode'+str(i)
        # path_name = str(Path(__file__).resolve().parents[0])+'/media/episode'+str(i)
